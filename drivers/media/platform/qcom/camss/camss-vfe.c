...
static const struct vfe_format formats_rdi_sdm845[] = {
	{ MEDIA_BUS_FMT_UYVY8_2X8, 8 },
	{ MEDIA_BUS_FMT_VYUY8_2X8, 8 },
	{ MEDIA_BUS_FMT_YUYV8_2X8, 8 },
	{ MEDIA_BUS_FMT_YVYU8_2X8, 8 },
...
static const struct vfe_format formats_pix_sdm845[] = {
	{ MEDIA_BUS_FMT_UYVY8_2X8, 8 },
	{ MEDIA_BUS_FMT_VYUY8_2X8, 8 },
	{ MEDIA_BUS_FMT_YUYV8_2X8, 8 },
	{ MEDIA_BUS_FMT_YVYU8_2X8, 8 },
			    ...
			    unsigned int index, u32 src_req_code)
{
	...
	if (vfe->camss->version == CAMSS_8x16)
		...
		default:
		 ...
		 vfe->camss->version == CAMSS_SDM845)
		switch (sink_code) {
		case MEDIA_BUS_FMT_YUYV8_2X8:
		{
			u32 src_code[] = {
			...
			const struct resources *res, u8 id)
{
	...
	for (i = VFE_LINE_RDI0; i <= VFE_LINE_PIX; i++) {
		...
		if (camss->version == CAMSS_8x16) {
			...
			if (i == VFE_LINE_PIX) {
				l->formats = formats_pix_sdm845;
				l->nformats = ARRAY_SIZE(formats_pix_sdm845);
			} else {
				l->formats = formats_rdi_sdm845;
				l->nformats = ARRAY_SIZE(formats_rdi_sdm845);
			}
		} else {
			return -EINVAL;
		}
