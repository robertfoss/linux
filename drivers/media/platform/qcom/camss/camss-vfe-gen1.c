 ...
 * camss-vfe-gen1.c
 *
 * Qualcomm MSM Camera Subsystem - VFE Common functionality for Gen 1 versions of hw (4.1, 4.7, 4.8, etc.)
 *
...
int vfe_gen1_halt(struct vfe_device *vfe)
{
	...
	time = wait_for_completion_timeout(&vfe->halt_complete, msecs_to_jiffies(VFE_HALT_TIMEOUT_MS));
	if (!time) {
...
static int vfe_enable_output(struct vfe_line *line)
{
	...
	if (line->id != VFE_LINE_PIX) {
		...
		vfe->ops_gen1->wm_set_ub_cfg(vfe, output->wm_idx[0], (ub_size + 1) * output->wm_idx[0], ub_size);
		...
		for (i = 0; i < output->wm_num; i++) {
			...
			vfe->ops_gen1->wm_set_ub_cfg(vfe, output->wm_idx[i], (ub_size + 1) * output->wm_idx[i], ub_size);
			vfe->ops_gen1->wm_line_based(vfe, output->wm_idx[i], &line->video_out.active_fmt.fmt.pix_mp, i, 1);
