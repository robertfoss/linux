...
static const struct resources csiphy_res_sdm845[] = {
	/* CSIPHY0 */
	{
		.regulator = { NULL },
...
static const struct resources csid_res_sdm845[] = {
	/* CSID0 */
	{
		.regulator = { "vdda-csi0" },
...
static const struct resources vfe_res_sdm845[] = {
	/* VFE0 */
	{
		.regulator = { NULL },
...
static int camss_configure_pd(struct camss *camss)
{
	...
	if (camss->version == CAMSS_8x96 ||
	    camss->version == CAMSS_SDM845) {
		camss->genpd[PM_DOMAIN_VFE0] = dev_pm_domain_attach_by_id(camss->dev, PM_DOMAIN_VFE0);
		if (IS_ERR(camss->genpd[PM_DOMAIN_VFE0]))
			return PTR_ERR(camss->genpd[PM_DOMAIN_VFE0]);

		camss->genpd[PM_DOMAIN_VFE1] = dev_pm_domain_attach_by_id(camss->dev, PM_DOMAIN_VFE1);
	...
	if (camss->version == CAMSS_SDM845) {
		camss->genpd[PM_DOMAIN_VFELITE] = dev_pm_domain_attach_by_id(camss->dev, PM_DOMAIN_VFELITE);
...
static int camss_probe(struct platform_device *pdev)
{
	...
	if (num_subdevs < 0) {
		dev_err(dev, "%s() camss_of_parse_ports() =%d\n", __func__, num_subdevs);
		ret = num_subdevs;
		goto err_cleanup;
	}
