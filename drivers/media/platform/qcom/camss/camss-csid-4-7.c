...
static void csid_configure_stream(struct csid_device *csid, u8 enable)
{
	...
	if (enable) {
		...
		if (tg->enabled) {
			...
			format = csid_get_fmt_entry(csid->formats, csid->nformats, input_format->code);
		...
		if ((sink_code == MEDIA_BUS_FMT_SBGGR10_1X10 && src_code == MEDIA_BUS_FMT_SBGGR10_2X8_PADHI_LE) ||
		    (sink_code == MEDIA_BUS_FMT_Y10_1X10 && src_code == MEDIA_BUS_FMT_Y10_2X8_PADHI_LE)) {
